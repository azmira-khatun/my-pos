...
@section('content')
<div class="container">
    <h2>Edit Expense: {{ $expense->reference ?? $expense->date->format('d M, Y') }}</h2>
    ...
    <form action="{{ route('expenses.update', $expense->id) }}" method="POST">
        ...
        <div class="form-group mb-3">
            ...
            <input type="text" name="reference" class="form-control @error('reference') is-invalid @enderror" value="{{ old('reference', $expense->reference) }}">
            @error('reference') <div class="invalid-feedback">{{ $message }}</div> @enderror
        </div>

        <div class="form-group mb-3">
            <label for="details">Details</label>
            <textarea name="details" class="form-control @error('details') is-invalid @enderror" rows="3">{{ old('details', $expense->details) }}</textarea>
